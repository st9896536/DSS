...
def decisiontree1(testindex,Tree):
    #判斷testindex 有沒有在node 裡面
    node = dict()
    #node['data'] = range(len(target[:100]))
    #tmp 把node dictionary轉成list
    tmp = list(range(100))
    if testindex in tmp:
...
def decisiontree2(testindex,Tree):
    ...
    while (t<len(Tree)):  #檢查i有沒有走道樹底 走到底就不走了！
        ...
        if(sum(target[idx])==1): #如果node[i]裡面全部人都不打球
            Tree[t]['leaf'] = 1 #1為葉節點打球
            Tree[t]['decision'] = 1 
        elif(sum(target[idx])==len(idx)): #如果全部都
            Tree[t]['leaf'] = 1
            Tree[t]['decision'] = 2
        ...
        if(bestIG>0):  #檢查最好的切法有沒有用
            ...
            if sum(target[idx] == 2) > sum(target[idx] == 1):
                Tree[t]['decision'] = 2 #採多數決 如果2多 就投2
            else:
                Tree[t]['decision'] = 1
        t+=1        
    ...
    while (t<len(Tree)):  #檢查i有沒有走道樹底 走到底就不走了！
        ...
        if(bestIG>0):  #檢查最好的切法有沒有用
            ...
            if sum(target[idx] == 0) > sum(target[idx] == 2):
                Tree[t]['decision'] = 0 #採多數決 如果0多 就投0
            else:
                Tree[t]['decision'] = 2
        t+=1        
...
for i in range(len(target)):
    ...
    decisiontree1(i,Tree1) #把每一筆測試資料拿去decisiontree
    decisiontree2(i,Tree2) #把每一筆測試資料拿去decisiontree
    decisiontree3(i,Tree3) #把每一筆測試資料拿去decisiontree

    arrpredict = [] #拿來存三棵樹測試出來的結果
    
    arrpredict.append(test(Tree1,i))
    arrpredict.append(test(Tree2,i))
    arrpredict.append(test(Tree3,i))
    ...
    #計算次數
    if target[i] != predictnum: #如果預測結果跟實際不一樣
        count+=1
print(count)
